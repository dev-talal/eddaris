...
import { RoundButton } from "../components/FormElements";
const QrResponse = () => {
  ...
  const blobGenerate = (url) => {
    fetch(url)
      ...
      .then((blob) => {
        ...
        setData({
          dataLink: URL.createObjectURL(blob),
          type: types.get(ext),
          ext: ext,
        });
        setLoader(false);
  ...
  useEffect(() => {
    ...
    const ext = query.get("url").split(/[#?]/)[0].split(".").pop().trim();
    if (["mp4", "mov", "3gp", "WebM", "flv"].includes(ext)) {
      setData({ dataLink: query.get("url"), type: types.get(ext), ext: ext });
      setLoader(false);
    } else {
              ...
              {!loader ? (
                <>
                  {data.type == "video" ? (
                    ...
                    <>
                      <FileText size={60} />
                      <p className="mt-2 mb-0 fbold">
                        <span className="text-capitalize">{data.ext} File</span>
                      </p>
                    </>
                  ) : data.type == "audio" ? (
                    <AudioPlayer autoPlay src={data.dataLink} />
                  ) : (
                    <>
                      <>
                        <FileText size={60} />
                  ...
                  <div className="col-sm-8 col-lg-6 col-12 mx-auto mt-5">
                    ...
                    <a
                      href="https://spreadthankyou.com/collections/all"
                      target="_blank"
                      className="f-16 text-white
                                            border-0 w-100 text-uppercase mt-3 d-flex
                    ...
                    >
                      <img
                        src={Cart}
                        className="align-self-center me-2 white-filter"
                        height="23px"
                      />
